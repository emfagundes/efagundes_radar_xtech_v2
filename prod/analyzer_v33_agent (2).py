...
Responda APENAS com o JSON, sem markdown, sem explicações."""
    ...
    try:
        ...
        # Guarda de palavra: manchete não pode passar de 9 palavras.
        # Se o LLM ignorou o limite, faz uma chamada de encurtamento.
        _palavras = manchete_final.split()
        if len(_palavras) > 9:
            print(f"     ⚠ manchete longa ({len(_palavras)} palavras) — reescrevendo...")
            try:
                _resp2 = client.messages.create(
                    model="claude-haiku-4-5-20251001",
                    max_tokens=80,
                    messages=[{"role": "user", "content":
                        f'Reescreva a manchete abaixo em NO MÁXIMO 9 PALAVRAS. '
                        f'Preserve verbo ativo, dado numérico e uma única ideia central. '
                        f'Responda APENAS com a manchete reescrita, sem aspas, sem explicação.\n\n'
                        f'Manchete original: {manchete_final}'}],
                )
                _m2 = _resp2.content[0].text.strip().strip('"').strip("'")
                if _m2 and len(_m2.split()) <= 12:
                    manchete_final = _m2
                    print(f"     ✓ manchete encurtada: '{manchete_final}'")
            except Exception as _e2:
                print(f"     ⚠ encurtamento falhou ({_e2}), truncando.")
                manchete_final = " ".join(_palavras[:9]).rstrip(",;:") + "…"
...
def main() -> None:
    ...
    # Passo B — enriquecimento Radar xTech (Fase 6.5, herdado de v30_agent)
    fase_65_enriquecimento_radar_xtech(client, DEFAULT_DB)

    # Passo C — enriquecimento Horizonte 1 (Fase 6.6, novo em v31)
    fase_66_horizonte1(client)
